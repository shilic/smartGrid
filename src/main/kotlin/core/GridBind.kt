...
fun GridBind.bindTo(kMutableProperty: KMutableProperty1<*,*>): GridInfo {
    return GridInfo.create(this, kMutableProperty)
}
/**  Grid 注解的扩展信息。
 *
 * =============================
...
class GridInfo private constructor (
    ...
    companion object {
        ...
        /**  通过工厂创建实例, 获取或创建GridInfo 。  */
        fun create (gridBind: GridBind, kMutableProperty: KMutableProperty1<*,*>): GridInfo {
            val cacheKey = createCacheKey(gridBind, kMutableProperty)
            val cached = cache[cacheKey]?.get()
            return cached ?: GridInfo( gridBind = gridBind, kMutableProperty = kMutableProperty, cacheKey = cacheKey)
                .also { newInstance -> cache[cacheKey] = WeakReference(newInstance)}
        }
        /**  创建唯一识别的缓存键  */
        fun createCacheKey(gridBind: GridBind, kMutableProperty: KMutableProperty1<*,*>): String {
            /* GridInfo 不仅仅需要记录注解信息，还需要使用字段的hash值联合作为唯一标识符，才能唯一识别一个注解。
             * 所以这里在最后边加了一句  kMutableProperty.hashCode() 。共同作为唯一标识符。*/
            return "${gridBind.headerText}_${gridBind.pattern}_${gridBind.valueType}_${gridBind.uiIgnore}_${gridBind.columnWidth}_${kMutableProperty.hashCode()}"
        }
        /** 清理无效的缓存引用 */
        @Suppress("UNUSED")
        fun cleanup() {
