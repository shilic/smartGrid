...
enum class GridValueType(
    ...
    NotDefined(-1, "未定义") {
        override fun parseGridCell(gridParser:IGridParser, cell: Cell?, sheet: Sheet, bind : GridColumnInfo, rowIndex: Ref<Int>, father: IGridRowData): Any {
    ...
    SubSignal(5, "子信号") {
        ...
        override fun parseGridCell(gridParser:IGridParser, cell: Cell?, sheet: Sheet, bind : GridColumnInfo, rowIndex: Ref<Int>, father: IGridRowData): Any {
            ...
            return gridParser.parseBySheet(sheet, valueClass, gridSheetType, rowIndex, father)
        }
    },
    ...
    OtherSheet(6, "子页面") {
        override fun parseGridCell(gridParser:IGridParser, cell: Cell?, sheet: Sheet, bind : GridColumnInfo, rowIndex: Ref<Int>, father: IGridRowData): Any {
            ...
            require(valueClass.isSubclassOf(IGridRowData::class)) { "类:\"${father::class.simpleName}\"的属性:\"${bind.kMutableProperty.name}\"中, 字典嵌套的值的类型也必须实现\"${IGridRowData::class.simpleName}\"接口 " }
            // 再将值的类型传入，进行嵌套调用
            return gridParser.parse(valueClass, father)
        }
    },
    ...
    SubStructure(9, "子结构") {
        ...
        override fun parseGridCell(gridParser: IGridParser, cell: Cell?, sheet: Sheet, bind: GridColumnInfo, rowIndex: Ref<Int>, father: IGridRowData): Any {
            ...
            val subStructures = gridParser.parseBySheet(sheet, valueClass, gridSheetType, rowIndex, father)
    ...
    SpecificSheet(10, "特定表格") {
        override fun parseGridCell(gridParser: IGridParser, cell: Cell?, sheet: Sheet, bind: GridColumnInfo, rowIndex: Ref<Int>, father: IGridRowData): Any {
            ...
            return gridParser.parseBySheet(newSheet, valueClass, gridSheetType, newRowIndex, father)
        }
    },
