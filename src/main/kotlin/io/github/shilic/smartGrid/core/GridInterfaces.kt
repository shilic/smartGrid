 ...
 * @param T 泛型T，需要实现 IGridData 接口。表示该 IGridSpecificSheet 接口负责管理何种类型的数据。
 * */
interface IMutableGridSpecificSheet<T : IMutableGridRowData> : IGridSpecificSheet<T>{
    /* 实现类自己需要定义自己的额外字段，以对应表格中的列标题。 */
    /** 在表格组件中的表格名称，框架会根据该变量的名称，到表格组件中去寻找对应的工作表。 */
    override val specificSheetName: String
    /** 持有的表格数据 */
    override val gridDataMap: MutableMap<String, T>
}
