package io.github.shilic.smartGrid.core

import org.apache.poi.ss.usermodel.Sheet
import org.apache.poi.ss.usermodel.Workbook
import kotlin.reflect.KClass

/** 一个接口，要求实现类必须给出一个字符串格式的唯一识别键，在生成字典的时候作为该实现类的字典的键 */
interface IGridKey {
    /** 表格数据的关键字。在生成字典的时候作为该实现类的字典的键。 */
    val gridKey : String
}


/**  不可变表格数据的子数据接口，用于指明该子数据的父级元素 */
interface IGridChild {
    /**  表格数据的子数据接口，用于指明该子数据的父级元素 */
    val gridFather : String
}
/**  可变表格数据的子数据接口，用于指明该子数据的父级元素 */
interface IMutableGridChild : IGridChild {
    /**  表格数据的子数据接口，用于指明该子数据的父级元素 */
    override var gridFather : String
}


/** 不可变表格数据行下标接口，用于记录对象的行下标 */
interface IGridRowIndex {
    /** 表格数据行下标接口，用于记录对象的行下标 */
    val gridRowIndex : Int?
}
/** 可变表格数据行下标接口，用于记录对象的行下标 */
interface IMutableGridRowIndex : IGridRowIndex {
    /** 表格数据行下标接口，用于记录对象的行下标 */
    override var gridRowIndex : Int?
}


/** 可变行数据接口。作用于类上，用于规范描述一行数据。
 *
 * 包括: 行数据的关键字、行数据的父级元素、行下标。 */
interface IMutableGridRowData : IGridKey, IMutableGridChild, IMutableGridRowIndex, IGridRowData

/** 不可变行数据接口。作用于类上，用于规范描述一行数据。
 *
 * 包括: 行数据的关键字、行数据的父级元素、行下标。 */
interface IGridRowData : IGridKey, IGridChild, IGridRowIndex


/** 不可变table接口。用于指示该数据是一个 完整表格 + 额外数据。
 *
 * 区别于直接在字段上使用 MutableMap 标记数据表。使用该接口之后，还可以在数据表之上，额外添加一些字段给这个对象。
 *
 * 再给 tableMap 字段的内部字段标记 GridBind 注解， GridBind 注解的属性设置为 GridValueType.SpecificTable， 表示从特定表格进行解析。
 * 表格名称从本接口的 specificTable 属性获取，而不是从 泛型 T 上边的表格注解 SheetBind 获取。
 *
 * 例如: 适用于有多个相同结构的表(例如多个DID，或者多个DBC)，但是你又不想使用字段来单独表示这些表，你想直接使用一个 Map 来表示这些相似的表，然后通过这些额外的数据来区分这些表，而不是字段名称和字段注解。
 *
 * */
interface IGridSpecificSheet {
    /* 实现类自己需要定义自己的额外字段，以对应表格中的列标题。 */
    /** 在表格组件中的表格名称，框架会根据该变量的名称，到表格组件中去寻找对应的工作表。 */
    val specificSheetName: String
}
/** 可变table接口。用于指示该数据是一个 完整表格 + 额外数据。
 *
 * 区别于直接在字段上使用 MutableMap 标记数据表。使用该接口之后，还可以在数据表之上，额外添加一些字段给这个对象。
 *
 * 再给 tableMap 字段的内部字段标记 GridBind 注解， GridBind 注解的属性设置为 GridValueType.SpecificTable， 表示从特定表格进行解析。
 * 表格名称从本接口的 specificTable 属性获取，而不是从 泛型 T 上边的表格注解 SheetBind 获取。
 *
 * 例如: 适用于有多个相同结构的表(例如多个DID，或者多个DBC)，但是你又不想使用字段来单独表示这些表，你想直接使用一个 Map 来表示这些相似的表，然后通过这些额外的数据来区分这些表，而不是字段名称和字段注解。
 *
 * */
interface IMutableGridSpecificSheet: IGridSpecificSheet {
    /* 实现类自己需要定义自己的额外字段，以对应表格中的列标题。 */
    /** 在表格组件中的表格名称，框架会根据该变量的名称，到表格组件中去寻找对应的工作表。 */
    override var specificSheetName: String
}


interface IGridReader {
    val workbook : Workbook
    val sheetMap: Map<String, Sheet>
    /** 注册表格值解析器 */
    fun registerGridValueAdapter(adapter: IGridValueAdapter)
    fun <T : Any> read(objectType: KClass<T>, father: IMutableGridRowData? = null): Map<String, T>
    fun <T : Any> readBySheet(sheet: Sheet, objectType: KClass<T>, gridSheetType: GridSheetType, rowIndex: Ref<Int>, father: IMutableGridRowData? = null): Map<String, T>
}



/** 不可变子数据拥有者, 使用行序号保存子数据对象 */
interface SubDataOwner {
    /**  使用行序号保存子数据对象 */
    val subDataMap : Map<Int, Any>
}
/** 可变子数据拥有者, 使用行序号保存子数据对象 */
interface MutableSubDataOwner : SubDataOwner {
    /**  使用行序号保存子数据对象 */
    override var subDataMap : MutableMap<Int, Any>
}


/**  不可变值描述， 以及值描述选项 */
interface IValueTable {
    /** 指示填入表格中的数据源，哪一个是值描述 */
    val valueTable : Map<Int, String>
    /** 值描述中具体取了哪一个值  */
    var aValue : String
}
/**  可变值描述， 以及值描述选项 */
interface IMutableValueTable : IValueTable {
    /** 指示填入表格中的数据源，哪一个是值描述 */
    override var valueTable : MutableMap<Int, String>
    /** 值描述中具体取了哪一个值  */
    override var aValue : String
}