...
import demo.dataModel.dbcDataModel.*
import demo.dataModel.udsDataModel.*
...
class MainTest {
    ...
    fun udsDataTest() {
        ...
        val ecus = GridReader(filePath.workbook()).read(Ecu::class)
    ...
    fun dbcDataTest() {
        ...
        // 使用路径实例化一个 workbook 并解析出数据。如果你想使用其他表格组件，使用适配器模式，让新组件实现 Workbook 系列接口即可实现任意表格的解析，不局限于EXCEL表格。
        val canProtocols: MutableMap<String, CanProtocol> = GridReader(filePath.workbook()).read(CanProtocol::class)
